  ...
  return (
    ...
    >
      ...
      >
        ...
        >{t("vehicle.cancel")}</Text>
      </TouchableOpacity>

      <Text style={styles.disabledText}>{title}</Text>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: Utilities.wp(8) }}>
        <View
          style={{
            flexDirection: "row",
        ...
        {options?.map((item, index) => {
          return (
            <TouchableOpacity style={[styles.option]} key={index} onPress={() => {selectOption(xmlName, item);}}>
              ...
              >
                ...
                <Image
                    style={{
                      ...
                      resizeMode: "contain",
                      tintColor: item.value === selectedValue ? Colors.DARKERGREEN : Colors.LIGHT_GREY,
                    }}
                    source={Mark}
                />
