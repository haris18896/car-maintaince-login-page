...
const SelectAddressBottomSheet = ({visible, closeSheet, serviceId, onPressConfirmAddress, addressToUpdate}) => {
        ...
        return (
            <View style={{flex: 1, justifyContent: 'space-between'}}>
                <MapView
                    ...
                    ref={refMap}
                    initialRegion={{
                        latitude: addressToUpdate ? Number(addressToUpdate?.latitude) : coordinates?.latitude,
                        longitude: addressToUpdate ? Number(addressToUpdate?.longitude) : coordinates?.longitude,
                        latitudeDelta: 0.0922,
                        longitudeDelta: 0.0421,
                    }}
    ...
    return (
        <BottomSheet
            ...
            <View style={{marginHorizontal: wp(20)}}>
                <InputWithLabel label={t("common.name")} placeholder={t("gorexOnDemand.selectAddressPlaceholder")} value={name} setValue={setName}/>
            </View>
            ...
            <View style={styles.bottomButton}>
                <RoundedSquareFullButton
                    ...
                    onPress={() => {
                        setName("");
                        onPressConfirmAddress(name, selectedCoordinates, nearbyServiceProviders, addressToUpdate?.id)
                    }} />
            </View>
