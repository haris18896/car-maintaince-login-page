...
const VehicleInformation = ({ route }) => {
  ...
  useEffect(() => {
    console.log('isComingFromOnDemand ===>> ', isComingFromOnDemand);
  ...
  const goToSuccessScreen = () =>{
    navigation.navigate("SuccessVehicle", {isComingFromOnDemand: isComingFromOnDemand});
  }
  ...
  return (
      <View style={styles.container}>
        ...
        <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
          ...
          <CustomBottomSheet open={numberPlateModal} roundedSquareFullButton title={t("vehicle.select")} onPress={selectNumberPlate} onClose={()=>setNumberPlateModal(false)} >
          ...
          <View style={styles.buttonContainer}>
            ...
            {image ? (
                <ImageBackground source={image ? { uri: `data:image/gif;base64,${image}` } : Istimara} style={styles.imagePicker} imageStyle={styles.istimaraBGImage}>
                  <TouchableOpacity style={styles.deleteBtn} onPress={() => setImage("")}>
                    <Text style={{ color: "white" }}> {t("vehicle.delete")}</Text>
                  </TouchableOpacity>
                </ImageBackground>
            ) : (
                <RoundedSquareFullButton title={t("vehicle.+AddIstimara")} onPress={handleImagePicker} />
            )}
          </View>
        </ScrollView>

        <Footer title={t("vehicle.confirmadd")} disabled={!cardAdded} onPress={vehicleToUpdate ? updateVehicle : addVehicle}/>
