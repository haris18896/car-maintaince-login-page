...
const OrderCard = ({ order, active }) => {
  ...
  const onPressOrder = () => {
    ...
    navigation.navigate("OrderDetails", {order_id:order.id, branch_id:order.service_provider[0], isOnDemand:false});
  }
      ...
      <View style={styles.paddedContent}>
        ...
        <View style={styles.row}>
          <Text style={[styles.status, {color: active === 1 ? Colors.DARKERGREEN : active === 2 ? Colors.ORANGE : Colors.RED,}]}>
            ...
            order?.status === "order_accepted" ? t('order_history.Incompleted') : order?.status}
          </Text>
